...
public Brick(int x, int y, int w, int h, int dx, int dy, 
		int xmin, int xmax, int ymin, int ymax, boolean isWanderer, Color color)
{
	this.x = x; 
	this.y = y; 
	this.w = w; 
...
{ 
	return "x=" + x + ", y=" + y + ", w=" +w + ", h=" +h + ", dx=" + dx + ", dy=" + dy +
			", color= " + color + ""; 
}
public boolean isUp() {
	return up;
...
private boolean checkCollisionRightSideOfRectangle(Brick r)
{
   ...
   if(x < r.x + r.w) {
         ...
         x = r.x + r.w;
         if(r.isWanderer)
         	r.dx = -r.dx;       //in case the colliding brick is moving, stop it from overlapping this brick the the edges of the panel
